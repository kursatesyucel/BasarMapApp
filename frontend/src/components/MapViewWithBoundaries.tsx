...
import { boundaryService, ProvinceDto, DistrictDto, SettlementDto } from '../services/boundaryService';
...
const MapViewWithBoundaries: React.FC<MapViewProps> = ({ mapFeatures }) => {
  ...
  const layersRef = useRef<{
    ...
    provinceBoundaries: L.LayerGroup;
    cityMarkers: L.LayerGroup;
    districtMarkers: L.LayerGroup;
  } | null>(null);
  ...
  const [districtCenters, setDistrictCenters] = useState<SettlementDto[]>([]);
  ...
  useEffect(() => {
    const loadBoundaryData = async () => {
      ...
      try {
        const [provinceData, citiesData, districtsData] = await Promise.all([
          boundaryService.getAllProvinces(),
          boundaryService.getCityAndCapitalCenters(),
          boundaryService.getSettlementsByCategory('İLÇE')
        ]);

        setProvinces(provinceData);
        setCityAndCapitalCenters(citiesData);
        setDistrictCenters(districtsData);
      } catch (error) {
  ...
  const handleLayerToggle = useCallback((layerKey: keyof LayerState) => {
    setLayerState(prev => ({
      ...prev,
  ...
  const handleOpacityChange = useCallback((layerKey: keyof LayerOpacity, value: number) => {
  ...
  const handleCitySelect = useCallback((settlement: SettlementDto) => {
    if (!mapInstanceRef.current) return;
    
    // FlyTo animation
    mapInstanceRef.current.flyTo([settlement.latitude, settlement.longitude], 10, {
      duration: 1.5,
      easeLinearity: 0.25
    });
    ...
    // Show popup
    const popup = L.popup()
      .setLatLng([settlement.latitude, settlement.longitude])
      .setContent(`
        <div class="popup-content">
          <h4>${settlement.name}</h4>
          <div class="popup-details">
            <div>Kategori: ${settlement.category}</div>
            <div>Lat: ${settlement.latitude.toFixed(6)}</div>
            <div>Lng: ${settlement.longitude.toFixed(6)}</div>
          </div>
        </div>
      `)
  ...
  useEffect(() => {
    ...
    const provinceBoundariesLayer = L.layerGroup().addTo(map);
    const cityMarkersLayer = L.layerGroup().addTo(map);
    const districtMarkersLayer = L.layerGroup().addTo(map);
    ...
    layersRef.current = {
      ...
      provinceBoundaries: provinceBoundariesLayer,
      cityMarkers: cityMarkersLayer,
      districtMarkers: districtMarkersLayer
    };
  ...
  useEffect(() => {
    if (!layersRef.current || provinces.length === 0) return;

    layersRef.current.provinceBoundaries.clearLayers();

    if (layerState.provinceBoundaries) {
      provinces.forEach(province => {
        try {
          const geojsonLayer = L.geoJSON(province.geometry, {
            style: {
              color: '#8b5cf6',
              weight: 2,
              opacity: layerOpacity.provinceBoundaries,
              fillOpacity: layerOpacity.provinceBoundaries * 0.1,
              fillColor: '#8b5cf6'
            }
          });

          geojsonLayer.addTo(layersRef.current!.provinceBoundaries);
        } catch (error) {
          console.error('Error rendering province boundary:', error);
        }
      });
    }
  }, [layerState.provinceBoundaries, layerOpacity.provinceBoundaries, provinces]);

  // City Markers Layer
  useEffect(() => {
    if (!layersRef.current || cityAndCapitalCenters.length === 0) return;

    layersRef.current.cityMarkers.clearLayers();

    if (layerState.cityMarkers) {
      cityAndCapitalCenters.forEach(city => {
        const marker = L.marker([city.latitude, city.longitude], { 
          icon: cityIcon,
          opacity: layerOpacity.cityMarkers
        });
        ...
        const popupContent = `
          <div class="popup-content">
            <h4>${city.name}</h4>
            <div class="popup-details">
              <div>Kategori: ${city.category}</div>
        ...
        marker.addTo(layersRef.current!.cityMarkers);
      });
    }
  }, [layerState.cityMarkers, layerOpacity.cityMarkers, cityAndCapitalCenters]);

  // District Markers Layer
  useEffect(() => {
    if (!layersRef.current || districtCenters.length === 0) return;

    layersRef.current.districtMarkers.clearLayers();

    if (layerState.districtMarkers) {
      districtCenters.forEach(district => {
        const marker = L.marker([district.latitude, district.longitude], { 
          icon: districtIcon,
          opacity: layerOpacity.districtMarkers
        });
        ...
        const popupContent = `
          <div class="popup-content">
            <h4>${district.name}</h4>
            <div class="popup-details">
              <div>Kategori: ${district.category}</div>
              <div>Lat: ${district.latitude.toFixed(6)}</div>
              <div>Lng: ${district.longitude.toFixed(6)}</div>
            </div>
          </div>
        `;

        marker.bindPopup(popupContent);
        marker.addTo(layersRef.current!.districtMarkers);
      });
    }
  }, [layerState.districtMarkers, layerOpacity.districtMarkers, districtCenters]);
  ...
  useEffect(() => {
    ...
    mapFeatures.points.forEach(point => {
      ...
      (marker as any)._pointId = point.id;
...
export default MapViewWithBoundaries;
